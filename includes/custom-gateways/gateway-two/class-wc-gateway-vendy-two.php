...
class WC_Gateway_Vendy_Two extends WC_Gateway_Custom_Vendy {
	...
	/**
	 * WC_Gateway_Vendy_Two constructor.
	 */
	public function __construct() {
		...
		if ( empty( $gateway_title ) ) {
			$gateway_title = __( 'Two', 'woo-vendy' );
		}

		$this->method_title       = sprintf( __( 'Vendy - %s', 'woo-vendy' ), $gateway_title );
		$this->method_description = sprintf( __( 'Vendy provide merchants with the tools and services needed to accept online payments from local and international customers using Mastercard, Visa, Verve Cards and Bank Accounts. <a href="%1$s" target="_blank">Sign up</a> for a Vendy account, and <a href="%2$s" target="_blank">get your API keys</a>.', 'woo-vendy' ), 'https://myvendy.com', 'https://dashboard.vendy.money/settings/webhooks' );
