...
class WC_Gateway_Custom_Vendy_Blocks_Support extends AbstractPaymentMethodType {

	/**
	 * Initializes the payment method type.
	 */
	public function initialize() {
		$vendy_gateway_settings        = get_option( 'woocommerce_vendy_settings', array() );
		$custom_vendy_gateway_settings = get_option( "woocommerce_{$this->name}_settings", array() );

		$this->settings = wp_parse_args( $custom_vendy_gateway_settings, $vendy_gateway_settings );
	...
	public function get_payment_method_script_handles() {
		$script_asset_path = plugins_url( "/assets/js/blocks/frontend/blocks/{$this->name}.asset.php", WC_VENDY_MAIN_FILE );
		$script_asset      = file_exists( $script_asset_path )
			? require $script_asset_path
			: array(
				'dependencies' => array(),
				'version'      => WC_VENDY_VERSION,
			);

		$script_url = plugins_url( "/assets/js/blocks/frontend/blocks/{$this->name}.js", WC_VENDY_MAIN_FILE );
		...
		if ( function_exists( 'wp_set_script_translations' ) ) {
			wp_set_script_translations( "wc-{$this->name}-blocks", 'woo-vendy', );
		}
	...
	public function get_payment_method_data() {
		...
		if ( empty( $payment_icons ) ) {
			$payment_icons = array( 'vendy' );
		}

		$payment_icons_url = array();
		foreach ( $payment_icons as $payment_icon ) {
			$payment_icons_url[] = WC_HTTPS::force_https_url( plugins_url( "assets/images/{$payment_icon}.png", WC_VENDY_MAIN_FILE ) );
		}
	...
	public function failed_payment_notice( PaymentContext $context, PaymentResult &$result ) {
		if ( $this->name === $context->payment_method ) {
			add_action(
				'wc_gateway_vendy_process_payment_error',
